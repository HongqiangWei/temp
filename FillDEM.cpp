...
void ProcessTraceQue(CDEM& dem,Flag& flag,queue<Node>& traceQueue, PriorityQueue& priorityQueue) 
{
	...
	while (!traceQueue.empty())
	{
		...
		noderow=node.row;
		nodecol=node.col;
		Mask[0][0]=Mask[0][1]=Mask[0][2]=Mask[0][3]=Mask[0][4]=false;
		Mask[1][0]=Mask[1][1]=Mask[1][2]=Mask[1][3]=Mask[1][4]=false;
		Mask[2][0]=Mask[2][1]=Mask[2][2]=Mask[2][3]=Mask[2][4]=false;
		Mask[3][0]=Mask[3][1]=Mask[3][2]=Mask[3][3]=Mask[3][4]=false;
		Mask[4][0]=Mask[4][1]=Mask[4][2]=Mask[4][3]=Mask[4][4]=false;
		for (i = 0; i < 8; i++){
